...
function Base(props) {
    ...
    var _d = __read(useState(false), 2), expanding = _d[0], setExpanding = _d[1];
    var _e = __read(useState(''), 2), ellipsisText = _e[0], setEllipsisText = _e[1];
    var _f = __read(useState(false), 2), measuring = _f[0], setMeasuring = _f[1];
    ...
    var calcEllipsisRef = useRef({
        expanding: expanding,
        ellipsis: isEllipsis,
        editing: mergedEditing,
    });
    var ellipsisConfig = ellipsis
        ? __assign({ rows: 1, ellipsisStr: '...' }, (isObject(ellipsis) ? ellipsis : {})) : {};
    ...
    function calcEllipsis() {
        ...
        if (ellipsisConfig.rows) {
            ...
            setMeasuring(true);
            var _a = measure(componentRef.current, ellipsisConfig, renderOperations(!!ellipsisConfig.expandable), children), ellipsis_1 = _a.ellipsis, text = _a.text;
            setMeasuring(false);
            if (ellipsis_1 && text) {
    ...
    function renderContent() {
        ...
        function renderInnerContent() {
            ...
            return (React.createElement(React.Fragment, null,
                isEllipsis && showTooltip && !expanding ? (React.createElement(TooltipComponent, __assign({ content: fullText }, tooltipProps), innerText)) : (innerText),
        ...
        return (React.createElement(TextComponent, __assign({ className: cs(prefixCls, componentClassName, className) }, baseProps, omit(rest, [
            'spacing',
            'type',
            'close',
            'bold',
    ...
    return (React.createElement(ResizeObserver, { onResize: resizeOnNextFrame }, mergedEditing ? (React.createElement(EditContent, __assign({}, props, { prefixCls: prefixCls, setEditing: setEditing, editableConfig: editableConfig }))) : (renderContent())));
}
export default Base;
